...
export class DialogComponent {
     ...
     * @param {string} [config.gridTemplateColumns] - [NEW] Optional CSS string for grid-template-columns.
     ...
     */
    show({ message, layout = [], gridTemplateColumns = '1fr 2fr 1fr', position = 'center', closeOnOverlayClick = true, onOpen = null }) {
        this.closeOnOverlayClick = closeOnOverlayClick;
        this.contentGridContainer.innerHTML = '';

        // [NEW] Apply dynamic grid layout
        this.contentGridContainer.style.gridTemplateColumns = gridTemplateColumns;
        ...
        layout.forEach(row => {
            row.forEach(cellConfig => {
                ...
                if (cellConfig.type === 'button') {
                    ...
                    // [MODIFIED] Use configured type, default to 'text'
                    ...
                    // [MODIFIED] Only add min=0 if type is actually number
                    if (input.type === 'number') {
                        input.min = 0;
                    }

                    // [MODIFIED] Add flag to disable default Enter confirmation
                    if (cellConfig.disableEnterConfirm !== true) {
                        input.addEventListener('keydown', (event) => {
                            if (event.key === 'Enter') {
                                event.preventDefault();
                                const confirmButton = this.contentGridContainer.querySelector('.primary-confirm-button');
                                if (confirmButton) {
                                    confirmButton.click();
                                }
                            }
                ...
                this.contentGridContainer.appendChild(cell);
            });
        });
