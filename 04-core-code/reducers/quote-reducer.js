...
export function quoteReducer(state, action, { productFactory, configManager }) {
    ...
    switch (action.type) {
        ...
        case QUOTE_ACTION_TYPES.RESTORE_K2_SNAPSHOT: {
            const { snapshotData } = action.payload;
            productData = { ...productData, items: snapshotData };
            return { ...state, products: { ...state.products, [productKey]: productData } };
        }
        ...
        case QUOTE_ACTION_TYPES.CYCLE_K2_PROPERTY: {
            items = [...productData.items];
